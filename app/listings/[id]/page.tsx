...
export default async function ListingPage({
  ...
  if (listing.bike_id) {
    ...
    const { data: componentsData } = await supabase
      .from("components")
      .select("id, type, brand, model, last_serviced_date, status, my_status")
      .eq("bike_id", listing.bike_id);
          ...
          <div className="md:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {bsbBike.brand && (
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{bsbBike.brand}</p>
                    </div>
                  )}
                  {bsbBike.model && (
                    <div>
                      <p className="text-gray-400 text-xs">Model</p>
                      <p className="font-medium text-gray-900">{bsbBike.model}</p>
                    </div>
                  )}
                  {bsbBike.year && (
                    <div>
                  ...
                  {bsbBike.is_ebike && (
                    <div>
                      ...
                      <p className="font-medium text-gray-900">E-Bike</p>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  {serviceEvents.map((event) => (
                    ...
                    >
                      <div className="text-xs text-gray-400 shrink-0 w-20 pt-0.5">
                        {event.service_date
                          ? new Date(event.service_date).toLocaleDateString("en-ZA", {
                              month: "short",
                              year: "numeric",
                            })
                          : "—"}
            ...
            {listing.has_bsb_history && (
              <div className="rounded-xl p-4" style={{ backgroundColor: "#EBF5FF" }}>
                <p className="font-semibold text-sm mb-1" style={{ color: "#2376BE" }}>
                  ✓ This bike has a verified BSB service history
                </p>
                <p className="text-xs text-gray-600">
                  When you buy this bike, pay R99 to claim the full service history
                  into your Bike Service Book account.
                </p>
              </div>
            )}
          </div>
